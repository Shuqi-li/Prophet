...
class Rhino_Informer(TorchModel,IModelForTimeseries):
    """
    ...
    def _create_dataset_for_sample(
        ...
        batch_size = infer_config_dict['batch_size']#64
        processed_dataset = self.data_processor.process_dataset(dataset)
    ...
    def _create_val_dataset_for_deci(
        ...
        val_dataset = MyTemporalTensorDataset(
            *to_tensors(val_data, device=self.device),
            lag=self.lag,
            is_autoregressive=True,
            index_segmentation=dataset._val_segmentation,
            time_span=self.pre_len
        )
    ...
    def run_train(
        ...
        # create dataloader for validation#change
        ...
        for step in range(train_config_dict["max_steps_auglag"]):
            ...
            for x, _ in dataloader:
                ...
                if batch_size * inner_step % 32 == 0: #change
                    log_step = len(str(round(num_samples/32)))
                    if round(num_samples/32)/(10^(log_step-1)) < 2:
                        log_step = log_step -1
            ...
            # mse_sample = self.run_inference_with_dataloader(val_dataloader, infer_config_dict)#change
            mse_sample = self.run_inference(dataset, infer_config_dict)

            # val_mse = mse_sample.mean()
            val_mse = mse_sample['test']['metric']['mse_mean']
            print(f"val_mse {val_mse}")

            nni.report_intermediate_result(val_mse)
    ...
    def _log_prior_A(self, A: torch.Tensor) -> torch.Tensor:
        ...
        sparse_term = -self.lambda_sparse * A.abs().sum()#change
        if self.exist_prior:
            prior_term = (
    ...
    def _ELBO_terms(self, X: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        log_p_A = self._log_prior_A(A_sample)  # A number #change exp
    ...
    def compute_loss(
        ...
        **kwargs,
        ...
        if train_config_dict["anneal_entropy"] == "linear":
            ELBO = log_p_term  - log_q_A_term / max(step - 5, 1) +  log_p_A_term/max(step - 5, 1)#change
        elif train_config_dict["anneal_entropy"] == "noanneal":
            ELBO = log_p_term  + log_q_A_term + log_p_A_term / max(step - 5, 1)#change p_A越来越大
        loss = -ELBO
