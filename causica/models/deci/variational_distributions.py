...
class TemporalThreeWayGrahpDist(nn.Module):
    """
    ...
    def __init__(
        ...
        self.Wx= generate_fully_connected(
            input_dim=input_dim,
            output_dim=input_dim*input_dim*2,
            hidden_dims=encoder_layer_sizes,
        ...
        self.Wg = generate_fully_connected(
            input_dim=2*input_dim*input_dim,
            output_dim=2*input_dim*input_dim,
            hidden_dims=encoder_layer_sizes,
            non_linearity=nn.LeakyReLU,
            activation=nn.Identity,
            device=self.device,
    ...
    #change
    def get_final_logits_lag(self, x_history):
        batch, lag, nodes = x_history.shape        

        x_history = x_history.flip([1]) #batch lag, nodes
        # logits_lag  (2, lag, input_dim, input_dim)  T->0
        # 生成x_item时，有node个分布生成器
        # x_item = torch.cat([self.Wx[i](x_history[i]) for i in range(nodes)], dim=-1) # batch lag, nodes*nodes*2    
        x_item = self.Wx(x_history) # batch lag, nodes*nodes*2
        tmp = torch.zeros_like(x_item) #(batch, nodes*nodes*2)
        tmp[:, 1:] = x_item[:, :-1].detach()
        tmp = tmp.view(batch, lag, -1)
        # tmp = tmp.transpose(0, 1).reshape(lag, nodes*nodes*2) #(lag-1, 2*nodes*nodes)
        self.final_logits_lag = (x_item +  self.Wg(tmp)).view(batch, lag, 2, nodes, nodes).transpose(1,2).transpose(0,1) # 2 batch lag nodes nodes
        #[2, batch, lag, nodes, nodes]

    def get_adj_matrix(self, x_history, do_round: bool = False) -> torch.Tensor:
        """
        ...
        Returns:
            The adjacency matrix with shape [batch, lag+1, num_nodes, num_nodes].
        """
        
        self.get_final_logits_lag(x_history)
  
        # Create the temporal adj matrix
        probs = torch.zeros(x_history.shape[0], self.lag, self.input_dim, self.input_dim, device=self.device)
        # Generate simultaneous adj matrix
        # Generate lagged adj matrix
        probs= F.softmax(self.final_logits_lag, dim=0)[1, ...]  # shape (batch, lag, input_dim, input_dim)
    ...
    def sample_A(self, x_history) -> torch.Tensor:
        ...
        adj_sample = torch.zeros(
            batch, self.lag, self.input_dim, self.input_dim, device=self.device
        )  # shape (lag+1, input_dim, input_dim)

    
        # Sample lagged adj matrix
        adj_sample = F.gumbel_softmax(self.final_logits_lag, tau=self.tau_gumbel, hard=True, dim=0)[
            1, ...
        ]  # shape (batch, lag, input_dim, input_dim)
        # T->0
        return adj_sample  #(batch, lag, input_dim, input_dim)
